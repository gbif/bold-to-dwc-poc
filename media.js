...
const extractMedia = async phylum => {
    ...
    const transformer = transform(function(record, callback){
        let images = ""
        if(record.phylum_name === phylum && record.image_urls){
            ...
            for(let i =0; i < identifiers.length; i++){
                ...
                images += `${record.processid}\t${record.processid} ${titles[i] ? titles[i] : ''}\t${identifiers[i]}\thttp://bins.boldsystems.org/index.php/Public_RecordView?processid=${record.processid}\timage/jpeg\t${licenses[i] || ''}\n`
